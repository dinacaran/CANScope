...
# Alternate spellings, resolved to the canonical spec. These are deliberately
# kept out of _FUNCTIONS: the Formula Help text is generated by iterating
# _FUNCTIONS.values(), so an alias entry there would list one function twice as
# if the two were unrelated. Every by-name lookup goes through
# _lookup_function() instead, so an alias behaves identically at parse and
# evaluation time.
_FUNCTION_ALIASES: dict[str, FunctionSpec] = {}
...
def _register(
    ...
    *,
    ...
    _FUNCTIONS[name.lower()] = spec
    for alias in spec.aliases:
        _FUNCTION_ALIASES[alias.lower()] = spec


def _lookup_function(name: str) -> FunctionSpec | None:
    """Resolve a call name, canonical or alias, to its spec."""
    key = name.lower()
    spec = _FUNCTIONS.get(key)
    if spec is not None:
        return spec
    return _FUNCTION_ALIASES.get(key)


def _known_function_names() -> list[str]:
    """Every spelling a formula may call, for did-you-mean suggestions."""
    return [*_FUNCTIONS, *_FUNCTION_ALIASES]
...
_register(
    "rolling_mean", 2, 2, "Time", "rolling_mean(signal, samples)",
    "Mean of the current and previous source samples over the same window as "
    "rolling_sum(); incomplete windows give NaN. Also spelled rolling_avg()",
    None, temporal=True, aliases=("rolling_avg",),
)
_register(
...
class _FormulaValidator(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        spec = _lookup_function(node.func.id)
        if spec is None:
            matches = difflib.get_close_matches(
                node.func.id.lower(), _known_function_names(), n=1
            )
...
def _current_value_tokens(node: ast.AST) -> set[str]:
    ...
    def visit(current: ast.AST) -> None:
        if isinstance(current, ast.Call) and isinstance(current.func, ast.Name):
            spec = _lookup_function(current.func.id)
...
def _rolling_window_sums(
    ...
    """Sum over the trailing `sample_count` source samples, held onto the grid.

    The window includes the current source sample. An incomplete window, or one
    holding any non-finite value, gives NaN; a window count larger than the
    source signal therefore gives NaN everywhere rather than raising.

    rolling_sum() and rolling_mean() both go through this, so their window
    boundaries and NaN rules cannot drift apart.
    """
    result = np.full(grid.size, np.nan, dtype=np.float64)
    if sample_count > source_values.size:
        return result

    source_indices = np.searchsorted(source_timestamps, grid, side="right") - 1
    ...
    with np.errstate(over="ignore", invalid="ignore"):
        prefix_sum = np.concatenate(([0.0], np.cumsum(safe_values, dtype=np.float64)))
    prefix_invalid = np.concatenate(([0], np.cumsum(invalid_values, dtype=np.int64)))
    ...
    if np.any(valid):
        ...
        window_invalid = prefix_invalid[ends] - prefix_invalid[starts]
        result[valid] = np.where(window_invalid == 0, window_values, np.nan)
    return result
...
def _evaluate_temporal_call(
    ...
    if function_name in {"lag", "rolling_sum", "rolling_mean"}:
        ...
        if function_name == "lag":
            ...
            if sample_count < 1:
                raise CalculatedSignalError(
                    f"{spec.name}() sample count must be greater than zero"
                )
            result = _rolling_window_sums(
                source_timestamps, source_values, context.grid, sample_count
            )
            if function_name == "rolling_mean":
                # sample_count is a positive int, so this cannot divide by zero;
                # NaN windows stay NaN.
...
def _evaluate_node(node: ast.AST, context: _EvaluationContext):
    ...
    if isinstance(node, ast.Call):
        spec = _lookup_function(node.func.id)
        if spec is None:  # pragma: no cover - the validator rejects unknown names
            raise CalculatedSignalError(f"Unknown function: {node.func.id}")
        if spec.temporal:
