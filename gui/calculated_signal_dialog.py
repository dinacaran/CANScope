...
def _formula_help_text() -> str:
    ...
    sections.append(
        ...
        "requires a complete positive-size window. rolling_mean(signal, samples) "
        "averages that same window and is also spelled rolling_avg. "
        "integral(signal) accumulates value x elapsed seconds using zero-order "
        "hold and starts at zero. derivative(signal) is its backward-difference "
        "counterpart, so the first source sample is NaN; unlike integral it is "
        "local, and one non-finite sample affects only the intervals touching "
        "it rather than everything after it.\n"
        "Every Time function takes a plain signal reference as its first "
        "argument, never an expression, so integral(`V` * `I`) is rejected. "
        "Build the expression as its own generated signal first, for example a "
        "signal Power with formula `V` * `I`, then reference that: "
        "integral(`Power`). Generated signals can be referenced by other "
        "formulas, so this two-step chain is the supported route rather than a "
        "workaround.\n"
